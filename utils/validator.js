...
const third_rule_allow = ["exist", "notexist", "enum", "array", "string", "number", "boolean", "object", "objectid", "file", "files", "params"];
...
const validate = async ({ rule, req, exclude_body = true, type = "body", version = 2, check_deleted_data = true, mongoose }) => {
  ...
  for (let i = 0; i < rules_key.length; i++) {
    ...
    if (split_rule_msg.length === 3) {
      ...
      if (!third_rule_allow.includes(third_rule[0])) {
        throw new Error(`Invalid thrid rule ${third_rule_allow}`);
      }
    }
