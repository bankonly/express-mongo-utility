...
const upload_file = async ({ file, bucket = process.env.AWS_S3_BUCKET_NAME, fileType = "jpg", path, complete = false, file_name, origin_filename = false, return_only_name = false }) => {
  try {
...
const upload_img_many = async ({ file, bucket = process.env.AWS_S3_BUCKET_NAME, fileType = "jpg", path, resize, complete = false }) => {
  try {
    if (!Array.isArray(file) || file.length < 1) throw new Error(`400::Multiple file required`);
    let result = [];
    for (let i = 0; i < file.length; i++) {
      const _file = file[i];
      const filename = await upload_img({ file: _file, bucket, fileType, path, resize, complete });
